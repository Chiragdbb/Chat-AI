...
- Local `npm run dev` loads `backend/.env` via Node's `--env-file`. Production `npm start` does not; hosts like Render inject env vars from the dashboard (do not commit `.env`).
